...
const dir = __dirname;
const inHtml = path.join(dir, "master_prep_exam_guide_v24_all_block_maps.html");
const outHtml = path.join(dir, "master_prep_exam_guide_v25_detailed_examples.html");
const outPdf = path.join(dir, "master_prep_exam_guide_v25_detailed_examples.pdf");
...
async function buildV24() {
  ...
  html = html.replace(/v24/g, "v25");
  html = html.replace(/Master Prep Exam Teaching Guide v24/g, "Master Prep Exam Teaching Guide v25");
  html = html.replace(/Master Prep Exam Guide v24/g, "Master Prep Exam Guide v25");
  html = html.replace(/PDF-версії v24/g, "PDF-версії v25");
  html = html.replace(/101-108/g, "101-109");
  html = html.replace(/4 \/ 108/g, "4 / 109");
  html = html.replace(
    ...
    `<b>🌍 Реальний приклад:</b> ліміт 32-bit integer став помітним, коли лічильник переглядів YouTube для “Gangnam Style” перетнув 2 147 483 647. Це максимум для signed 32-bit integer: <code>2^31 - 1</code>.
        <div class="v24-flow">Було:
...
верхня межа: 2^63 - 1
= 9 223 372 036 854 775 807</div>
        <p><b>Що саме змінили:</b> не “магічну формулу”, а тип/розмір поля, у якому зберігався лічильник: з 32 бітів пам'яті для числа на 64 біти. Тобто для одного значення стало доступно вдвічі більше бітів, але кількість можливих значень виросла не вдвічі, а експоненційно: з приблизно 2,1 млрд до приблизно 9,22 квінтильйона.</p>
        <p><b>Чому signed, якщо перегляди не бувають від'ємними?</b> У багатьох мовах, базах даних і API стандартний “великий integer” часто є signed int64 / BIGINT. Для лічильника можна було б теоретично взяти unsigned 64-bit, тоді максимум був би <code>2^64 - 1 = 18 446 744 073 709 551 615</code>, але публічно YouTube згадував саме 64-bit integer із межею <code>2^63</code>/<code>2^63 - 1</code>, тобто по суті signed 64-bit.</p>
        <p><b>Альтернатива ще більша:</b> для зовсім необмежених чисел у деяких системах використовують arbitrary precision / BigInt, але для лічильника переглядів 64-bit достатньо з величезним запасом.</p>`
  );
