...
const dir = __dirname;
const inHtml = path.join(dir, "master_prep_exam_guide_v25_detailed_examples.html");
const outHtml = path.join(dir, "master_prep_exam_guide_v26_efvv_technology.html");
const outPdf = path.join(dir, "master_prep_exam_guide_v26_efvv_technology.pdf");
...
async function printPdfWithFooter(chrome, htmlPath, pdfPath) {
  ...
  const profileDir = path.join(process.env.TEMP || dir, `chrome-print-v26-${Date.now()}`);
...
async function main() {
  ...
  html = html.replace(/master_prep_exam_guide_v25_detailed_examples/g, "master_prep_exam_guide_v26_efvv_technology");
