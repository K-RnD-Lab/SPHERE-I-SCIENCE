...
async function loadResources() {
  try {
      const res = await fetch('./analytics-data/resource_catalog.csv', { cache: 'no-store' });
    if (!res.ok) return [];
...
async function loadLocalCsvData() {
  const [studyRows, sessionRows] = await Promise.all([
    fetch('./analytics-data/study_log_template.csv', { cache: 'no-store' }).then((res) => (res.ok ? res.text() : '')).then(parseCsv).catch(() => []),
    fetch('./analytics-data/session_log_template.csv', { cache: 'no-store' }).then((res) => (res.ok ? res.text() : '')).then(parseCsv).catch(() => []),
  ]);
